...
<?php
// SMSCENTRE.COM API (smscentre.com) версия 3.4 (05.03.2015)

define("SMSC_LOGIN", get_conf_param('smsc_login'));            // логин клиента
define("SMSC_PASSWORD", get_conf_param('smsc_pass'));    // пароль или MD5-хеш пароля в нижнем регистре
...
{
    return mail("send@send.smscentre.com", "", SMSC_LOGIN.":".SMSC_PASSWORD.":$id:$time:$translit,$format,$sender:$phones:$message", "From: ".SMTP_FROM."\nContent-Type: text/plain; charset=".SMSC_CHARSET."\n");
}
...
{
    $url = (SMSC_HTTPS ? "https" : "http")."://smscentre.com/sys/$cmd.php?login=".urlencode(SMSC_LOGIN)."&psw=".urlencode(SMSC_PASSWORD)."&fmt=1&charset=".SMSC_CHARSET."&".$arg;
    ...
    do {
        if ($i) {
            ...
            if ($i == 2)
                $url = str_replace('://smscentre.com/', '://www2.smscentre.com/', $url);
        }
