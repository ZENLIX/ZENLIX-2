...
var socket = io.connect(location.protocol+'//'+show_hostname(MyHOSTNAME)+':'+NODE_PORT, {secure: true});


socket.emit('join', {uniq_id : USER_HASH});
...
//push_msg_action2user
		socket.on("new_msg", function(data) {
					...
					switch (data.type_op) { 
						case 'ticket_create': active_noty_msg('ticket_create', data.t_id);update_labels();
						  if ((def_filename == "index.php") || (window.location == MyHOSTNAME)) {
                                       update_page_dashboard();makemytime(true); update_dashboard_labels();
						  };
						  if (ispath('list')) { update_list_page_content();makemytime(true); };
						  
						break;
						
						
						case 'ticket_refer': active_noty_msg('ticket_refer', data.t_id);update_labels();
						  if ((def_filename == "index.php") || (window.location == MyHOSTNAME)) {
                                       update_page_dashboard();makemytime(true);update_dashboard_labels(); 
						  };
						  if (ispath('list')) { update_list_page_content();makemytime(true); };
						  
						  if (ispath('ticket')) { update_ticket_page(data.t_id); };
						  
						  
						break;
						
						
						case 'ticket_ok': active_noty_msg('ticket_ok', data.t_id);
						  if ((def_filename == "index.php") || (window.location == MyHOSTNAME)) {
                                       update_page_dashboard();makemytime(true);update_labels();update_dashboard_labels(); 
						  };
						  if (ispath('list')) { update_list_page_content();makemytime(true);update_labels(); };
						  if (ispath('ticket')) { update_ticket_page(data.t_id);update_labels(); };
						break;
						
						
						case 'ticket_no_ok': active_noty_msg('ticket_no_ok', data.t_id);
						...
						case 'ticket_lock': active_noty_msg('ticket_lock', data.t_id);
						  if ((def_filename == "index.php") || (window.location == MyHOSTNAME)) {
                                       update_page_dashboard();makemytime(true); update_dashboard_labels();
						  };
						  if (ispath('list')) { update_list_page_content();makemytime(true); };
						...
						case 'ticket_unlock': active_noty_msg('ticket_unlock', data.t_id);
						  if ((def_filename == "index.php") || (window.location == MyHOSTNAME)) {
                                       update_page_dashboard();makemytime(true);update_dashboard_labels(); 
						  };
						  if (ispath('list')) { update_list_page_content();makemytime(true); };
						...
						case 'logout': window.location = MyHOSTNAME+"index.php?logout";
						break;
						};
  ...
  scroll.scrollTop(height);
    ...
    $("input#msg").keyup(function() {
        if($(this).val().replace(/ /g,'').length > 1) {
            ...
            setTimeout(function(){ $("input#msg").popover('hide'); }, 2000);
        }
    });
...
$.ajax({
                type: "POST",
                url: ACTIONPATH,
                data: "mode=messages_title_username"+
                "&uid="+user_id,
...
$.ajax({
                type: "POST",
                url: ACTIONPATH,
                data: "mode=recalculate_messages_ul"+
                "&uid="+user_id,
                success: function(html) {
                $(el).html(html);
                show_bar_unread_msg();
    ...
    $('body').on('click', 'button#do_comment', function(event) {
        ...
        if($("input#msg").val().replace(/ /g,'').length > 1) {
            ...
            $.ajax({
                type: "POST",
                url: ACTIONPATH,
                data: "mode=messages_send"+
                    "&textmsg="+encodeURIComponent(($("input#msg").val()))+
                    "&target="+tid,
                success: function(html) {
  ...
  $('#form_avatar').submit();
	...
	$('body').on('click', 'button#edit_profile_main_client', function(event) {
        ...
        $.ajax({
            type: "POST",
            url: ACTIONPATH,
            data: "mode=edit_profile_main_client"+
                ...
                "&adr="+encodeURIComponent($("#adr").val())+
                "&id="+encodeURIComponent($("#edit_profile_main").attr('value')),
            success: function(html) {

                $("#m_info").hide().html(html).fadeIn(500);
				setTimeout(function() {$('#m_info').children('.alert').fadeOut(500);}, 3000);
...
if ($("#fio").val().length < 3) {
        ...
        $.ajax({
            ...
            data: "mode=edit_profile_main"+
                ...
                "&tel="+encodeURIComponent($("#tel").val())+
                "&adr="+encodeURIComponent($("#adr").val())+
                "&posada="+encodeURIComponent($("#posada").val())+
                "&unit="+encodeURIComponent($("#pidrozdil").val())+
                "&id="+encodeURIComponent($("#edit_profile_main").attr('value')),
            success: function(html) {
    ...
    $('body').on('click', 'button#edit_profile_pass', function(event) {
        ...
        $.ajax({
            ...
            success: function(html) {
				...
				setTimeout(function() {$('#p_info').children('.alert').fadeOut(500);}, 3000);
...
//console.log(html);
    ...
    $("select#to").change(function() {
        ...
        if ( $('select#to').val() != 0 ){
            ...
            $('#for_to').popover('show');
            $('#for_to').addClass('has-error');
             setTimeout(function(){ $("#for_to").popover('hide'); }, 2000);
...
*/
           ...
           $("#client_id_param").val(item.p);
            ...
            $.ajax({
                ...
                success: function(html) {
                    $("#user_info").hide().html(html).fadeIn(500);
$('#edit_login').editable({type: 'text', pk: 1, url: MyHOSTNAME+'actions.php',inputclass: 'input-sm', emptytext: 'пусто', params: {
...
makemytime(true);
                    ...
                    $('#for_fio').addClass('has-success');
                        ...
                        success: function(html) {
                            ...
                            $('#new_login').editable({type: 'text', pk: 1, url: MyHOSTNAME+'actions.php',inputclass: 'input-sm', emptytext: 'пусто', params: {
        mode: 'verify_login_nt'
    }});
  ...
  /*      setInterval(function(){
    ...
    $('body').on('click', 'button#do_save_help', function(event) {
        ...
        var t=$("#t").val();
        var data = { 'mode' : 'do_save_help', 'u' : u, 't' : t, 'msg' : sHTML, 'hn': hn, 'is_client':is_client };
    ...
    $('body').on('click', 'button#do_create_help', function(event) {
        ...
        var t=$("#t").val();
        var data = { 'mode' : 'do_create_help', 'u' : u, 't' : t, 'msg' : sHTML, 'is_client':is_client };
	...
	$("textarea#msg").keyup(function() {
        if($(this).val().length > 1) {
            ...
            $("textarea#msg").popover('show');
            setTimeout(function(){ $("textarea#msg").popover('hide'); }, 2000);
            $("#for_msg").addClass('has-error');
        }
    });
	/*
...
makemytime(true);
	...
	if (ispath('config') ) {
        ...
        $.ajax({
            ...
            data: "mode=check_version",
				...
				setTimeout(function() {$('#result_update').children('.alert').fadeOut(500);}, 3000);
	...
	$('body').on('click', 'button#conf_edit_pb', function(event) {
        ...
        $.ajax({
            type: "POST",
            url: ACTIONPATH,
            data: "mode=conf_edit_pb"+
                "&api="+encodeURIComponent($("input#pb_api").val()),
                success: function(html) {
                $("#conf_edit_pb_res").hide().html(html).fadeIn(500);
				setTimeout(function() {$('#conf_edit_pb_res').children('.alert').fadeOut(500);}, 3000);
	...
	$('body').on('click', 'button#conf_edit_main', function(event) {
        ...
        $.ajax({
            ...
            data: "mode=conf_edit_main"+
                ...
                "&ldap="+encodeURIComponent($("input#ldap_ip").val())+
                "&ldapd="+encodeURIComponent($("input#ldap_domain").val())+
...
bootbox.confirm(get_lang_param('JS_del'), function(result) {
	...
	//push_msg_action2user
        ...
        $.ajax({
            type: "POST",
            url: ACTIONPATH,
            data: "mode=push_msg_action2user"+
                "&user="+encodeURIComponent(usid)+
                "&op=logout",
            success: function(html) {
                //alert(html);
    ...
    $("input#fio_user").keyup(function() {
        ...
        $.ajax({
            ...
            data: "mode=add_user"+
                ...
                "&ldap_auth_key="+encodeURIComponent($("#ldap_auth_key").prop('checked'))+
                "&mess="+encodeURIComponent($("textarea#mess").val())+
                "&mess_t="+encodeURIComponent($("input#msg_title").val())+
                "&push="+encodeURIComponent($("input#push").val())+
                "&tel="+encodeURIComponent($("input#tel").val())+
                "&skype="+encodeURIComponent($("input#skype").val())+
                "&adr="+encodeURIComponent($("input#adr").val())+
                "&posada="+encodeURIComponent($("#posada").val())+
                "&pidrozdil="+encodeURIComponent($("#pidrozdil").val())+
    ...
    $('body').on('click', 'button#edit_user', function(event) {
        ...
        $.ajax({
            ...
            data: "mode=edit_user"+
                "&fio="+encodeURIComponent($("#fio_user").val())+
                "&login="+encodeURIComponent($("#login_user2").val())+
                "&pass="+encodeURIComponent($("#exampleInputPassword1").val())+
                "&unit="+encodeURIComponent($("#my-select").val())+
                "&priv="+encodeURIComponent($("input[type=radio][name=optionsRadios]:checked").val())+
                "&ldap_auth_key="+encodeURIComponent($("#ldap_auth_key").prop('checked'))+
