...
function get_user_lang(){
    ...
    if (($length < 1) || $max_id == "0") {$ress='en';} else {$ress=$max_id;}
    return $ress;
}
...
//$file = explode("?", basename($file));
    ...
    if ($current_file_name == $requestUri) { return true;}
    else {return false;}
}
...
$ldaprdn  = $login.'@'.get_conf_param('ldap_domain');     // ldap rdn or dn
$ldappass = $pass;  // associated password

// connect to ldap server
$ldapconn = ldap_connect(get_conf_param('ldap_ip'))
...
$ru=name_of_user_ret($row['init_user_id']);
                                                ...
                                                <tr>
                                                    ...
                                                    <a href="view_user?<?=get_user_hash_by_id($row['init_user_id']);?>"><?=$ru?>
                                                    </a>
                                                    </center></small></td>
...
$ct=make_html($rews['msg'], true);
        ...
        ?>
                                            <!-- chat item -->
                                    <div class="item" style=" min-height: 35px; <?=$s;?>">
                                        <img src="<?=get_user_img_by_id($rews['user_from']);?>" alt="user image" class="<?=get_user_status_text($rews['user_from']);?>"/>
                                        <div class="message">
                                        <small class="text-muted pull-right"><i class="fa fa-clock-o"></i> 
                                                <time id="b" datetime="<?=$rews['date_op'];?>"></time> <time id="c" datetime="<?=$rews['date_op'];?>"></time>
                                                </small>
                                            <a href="view_user?<?=get_user_hash_by_id($rews['user_from']);?>" class="name">
...
function view_comment($tid) {
    ...
    ?>
        ...
        while ($rews = $stmt->fetch(PDO::FETCH_ASSOC)) { 
	       ...
	       else {$ct=make_html($rews['comment_text'], true);
        ...
        ?>
                                    ...
                                    <div class="item" style=" min-height: 35px; ">
                                        <img src="<?=get_user_img_by_id($rews['user_id']);?>" alt="user image" class="<?=get_user_status_text($rews['user_id']);?>"/>
                                        <div class="message">
                                        <small class="text-muted pull-right"><i class="fa fa-clock-o"></i> 
                                                <time id="b" datetime="<?=$rews['dt'];?>"></time> <time id="c" datetime="<?=$rews['dt'];?>"></time>
                                                </small>
                                            <a href="view_user?<?=get_user_hash_by_id($rews['user_id']);?>" class="name">
...
if (get_user_authtype($ul)) {
	...
	else if (ldap_auth($ul,$input) == false) {return false;}
        ...
        <?php
            ...
            foreach ($result as $row) {
                ...
                if ($ac == "ok") {
                    ...
                    ?>
                    <tr><td><small><i class="fa fa-file-text-o"></i> </small><a href="helper?h=<?= $row['hashname']; ?>"><small><?= cutstr_help2_ret($row['title']); ?></small></a></td><td><small style="float:right;" class="text-muted">(<?= lang('DASHBOARD_author'); ?>: 
                    <a href="view_user?<?=get_user_hash_by_id($row['user_init_id']);?>">
                    <?= nameshort(name_of_user_ret($row['user_init_id'])); ?>)
                    </a>
...
if ($fio['usr_img']) {
    ...
    else if (!$fio['usr_img']) {$user_img=$CONF['hostname'].'/img/avatar5.png';}
...
<div class="box box-info">
                                ...
                                <div class="box-body">
                                    ...
                                    <div class="row">
                                <div class="col-md-5"><img class="img-rounded" src="<?=$user_img;?>" height="120"></div>
                                <div class="col-md-7">
                                <h4><center><strong><?php echo $fio_user; ?></strong></center></h4>
                                 ...
                                 <small class="text-muted"><?=lang('WORKER_total');?>:</small>
                                 <small>
                        <?php if ($priv_val <> "1") { ?>
...
<?php if ( $tt <> 0) { ?>
            ...
            <small class="text-muted"><?=lang('WORKER_last');?>:</small>
               <small><?php if ($priv_val <> "1") { ?><a target="_blank" href="userinfo?user=<?=$id?>"><?php } ?>
                
                <time id="b" datetime="<?=$lt;?>"></time>
                <time id="c" datetime="<?=$lt;?>"></time>
                
                <?php if ($priv_val <> "1") { ?></a><?php } ?></small>
...
$id=$_SESSION['helpdesk_user_id'];
   ...
   <div class="box box-info">
                                ...
                                <div class="col-md-5"><img class="img-rounded" src="<?=$user_img;?>" height="120"></div>
                                <div class="col-md-7">
                                <h4><center><strong><?php echo $fio_user; ?></strong></center></h4></div>
                                </div>
                                <br>
...
<div class="box box-info">
               ...
               <small class="text-muted">
                        <?php if ($priv_val <> "1") { ?>
                        <a target="_blank" href="userinfo?user=<?=$id?>"><?php }?><?php echo $tt; ?><?php if ($priv_val <> "1") { ?></a><?php } ?></small><br>
<?php if ( $tt <> 0) { ?>
           <small class="text-muted"><?=lang('WORKER_last');?>:</small>
                <td><small class="text-muted">
                        <?php if ($priv_val <> "1") { ?>
...
if (isset($_SESSION['hd.rustem_sort_in'])) {
	if ($_SESSION['hd.rustem_sort_in'] == "ok"){$stmt = $dbConnection->prepare("SELECT max(last_update) from tickets where ((find_in_set(:id,user_to_id) and arch='0') or (user_to_id='0' and unit_id IN (".$in_query.") and arch='0')) and status=:s");
			...
			$max_id=$max[0];
				...
				else if ($_SESSION['hd.rustem_sort_in'] == "free"){$stmt = $dbConnection->prepare("SELECT max(last_update) from tickets where ((find_in_set(:id,user_to_id) and arch='0') or (user_to_id='0' and unit_id IN (".$in_query.") and arch='0')) and lock_by=:lb and status=:s");
            $paramss=array(':id' => $id, ':lb'=>'0', ':s'=>'0');
            $stmt->execute(array_merge($vv,$paramss));
			$max = $stmt->fetch(PDO::FETCH_NUM);
			$max_id=$max[0];}
			
	else if ($_SESSION['hd.rustem_sort_in'] == "ilock"){$stmt = $dbConnection->prepare("SELECT max(last_update) from tickets where ((find_in_set(:id,user_to_id) and arch='0') or (user_to_id='0' and unit_id IN (".$in_query.") and arch='0')) and lock_by=:lb");
	...
	else if ($_SESSION['hd.rustem_sort_in'] == "lock"){$stmt = $dbConnection->prepare("SELECT max(last_update) from tickets where ((find_in_set(:id,user_to_id) and arch='0') or (user_to_id='0' and unit_id IN (".$in_query.") and arch='0')) and (lock_by<>:lb and lock_by<>0) and (status=0)");
            $paramss=array(':id' => $id, ':lb'=>$lb);
            $stmt->execute(array_merge($vv,$paramss));
			$max = $stmt->fetch(PDO::FETCH_NUM);
			$max_id=$max[0];}
...
if (!isset($_SESSION['hd.rustem_sort_in'])) {            $stmt = $dbConnection->prepare("SELECT max(last_update) from tickets where ((find_in_set(:id,user_to_id) and arch='0') or (user_to_id='0' and unit_id IN (".$in_query.") and arch='0'))");
            $paramss=array(':id' => $id);
            $stmt->execute(array_merge($vv,$paramss));
			$max = $stmt->fetch(PDO::FETCH_NUM);
			$max_id=$max[0];
