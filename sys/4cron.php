...
$type = "ticket_create";
            ...
            foreach ($delivers_ids as $uniq_id_row) {
                ...
                $stmt_n->execute(array(':delivers_id' => $u_hash, ':type_op' => 'ticket_create', ':tid' => $ticket_id, ':n' => $now_date_time));
            }
            ...
            $stmt->execute(array(':delivers_id' => $res_str, ':type_op' => $type, ':tid' => $ticket_id, ':n' => $now_date_time));
        } 
        else if ($user_to_id <> 0) {
            ...
            if ($su) {
                ...
                $stmt = $dbConnection->prepare('insert into notification_pool (delivers_id, type_op, ticket_id, dt) VALUES (:delivers_id, :type_op, :tid, :n)');
                $stmt->execute(array(':delivers_id' => $su, ':type_op' => $type, ':tid' => $ticket_id, ':n' => $now_date_time));
            }
        }
