...
<div class="box box-default">
   ...
   <div class="row">
            ...
            <div class="col-md-6">
<?=view_post_buttons_status($post_id);?>
            </div><!-- /.col -->
            <div class="col-md-6">

<div class="btn-group-vertical ">
<button class="btn btn-xs bg-maroon main-post-del" value="<?=$post_hash;?>"><?=lang('PORTAL_act_del');?></button> 
<button class="btn btn-xs bg-orange btn-flat main-post-edit" value="<?=$post_hash;?>"><?=lang('PORTAL_act_edit');?></button>
 </div>
...
<div class="box box-default">
                <div class="box-header with-border">
                  <h3 class="box-title"><?=lang('PORTAL_maybe_theme');?></h3>
 ...
 ?>
                    <li class="item">
                      ...
                      <div class="product-info" style="margin-left:0px;">
                        ...
                        <i class="fa fa-clock-o"></i> <time id="d" datetime="<?=$k['dt'];?>"></time>
                        </small>
                      </div>
                    </li><!-- /.item -->
                            <?php
                            ...
                            ?>           
                    <li><a href="feed"><small class="text-muted"><?=lang('PORTAL_all_news');?> (<?=get_count_news();?>)</small></a></li>
                  </ul>
...
<div class="box box-default">
                <div class="box-header with-border">
                  <h3 class="box-title"><?=lang('PORTAL_stat');?></h3>
...
$stmt = $dbConnection->prepare('SELECT * from post_likes where post_id=:tm and user_id=:u');
    ...
    if ($tt['likes']) {
    	...
    	if ($v == "2") { $s['like']="disabled";}
    	else if ($v == "1") { $s['dislike']="disabled";}
...
?>
<div class="btn-group">
                          <button type="button" id="do_like" value="like" class="btn btn-xs btn-success <?=$s['like'];?>"><i class="fa fa-thumbs-o-up"></i> <?=lang('PORTAL_post_like');?></button>
                          <button type="button" id="do_like" value="dislike" class="btn btn-xs btn-danger <?=$s['dislike'];?>"><i class="fa fa-thumbs-o-down"></i> <?=lang('PORTAL_post_dislike');?></button>
                        </div>
<?php
}
}
...
if (($row['parent_id'] == "0") && (($row['type'] == "1"))) {echo "<br>";}
...
if ($level != 0) { echo "<ul>"; }
    else if ($level == 0) { echo "<ul style=\"padding-left: 5px;\">"; }

   // while ($node = mysql_fetch_array($result)) { 
    foreach ($re as $row) {

if (($row['parent_id'] == "0") && (($row['type'] == "1"))) {echo "<i class=\"fa fa-graduation-cap\"></i>";}
...
if ($row['is_success'] == "1") {
$style_act="list-group-item-success"; $style_icon="fa-check-circle";
}
else if ($row['is_success'] == "0") {
	$style_act=""; $style_icon="fa-clock-o";
}
...
if ($level != 0) { echo "<ul>"; }
   ...
   // while ($node = mysql_fetch_array($result)) { 
    ...
    if ($row['is_success'] == "1") {$c1="checked";}
    	else if ($row['is_success'] == "0") {$c1="";}
        ...
        ?>
                                        ...
                                        <a href="#" data-pk="<?=$row['id'];?>" data-url="inc/main_portal/actions.php" id="edit_item_todo" data-type="text" style="width:400px;">
                                                <?=$row['name'];?>
                                            </a> 
                                            </span>
                    ...
                    <input id="make_todo_success" name="" value="<?=$row['id'];?>" type="checkbox" <?=$c1;?>> <small><?=lang('PORTAL_success');?></small>
                </label>    
                   <!--i id="edit_todo" value="<?=$row['uniq_id'];?>" class="fa fa-pencil-square-o"></i-->
                 
                
            <i id="del_item_todo" value="<?=$row['id']?>" class="fa fa-trash-o"></i>
    ...
    foreach ($re as $row) {
        ...
        ?>
                                        <li id="list_<?=$row['id'];?>">
                                            ...
                                            <!-- todo text -->
                                            <span class="text" id="val_<?=$row['id'];?>">
                                        <a href="#" data-pk="<?=$row['id'];?>" data-url="inc/main_portal/actions.php" id="edit_item_qa" data-type="text" class="">
                                                <?=$row['question'];?>
                                            </a> 
                                            </span>
                ...
                <i id="edit_manual_qa" value="<?=$row['uniq_id'];?>" class="fa fa-pencil-square-o"></i>
            <i id="del_item_qa" value="<?=$row['id']?>" class="fa fa-trash-o"></i>
...
if ($i==0) {$fel="in";}
                    ...
                    <div class="panel box box-default">
                      <div class="box-header with-border">
                        <h4 class="box-title">
                          <a data-toggle="collapse" data-parent="#accordion" href="#collapse_<?=$row['uniq_id'];?>">
                            <?=$row['question'];?>
                          </a>
                        </h4>
                        <div class="box-tools pull-right">
...
if ($level != 0) { echo "<ul>"; }
   ...
   // while ($node = mysql_fetch_array($result)) { 
    foreach ($re as $row) {
    	...
    	if ($row['main'] == "1") {$c="checked";}
    	else if ($row['main'] == "0") {$c="";}
        ...
        ?>
                                        <li id="list_<?=$row['id'];?>">
                                            ...
                                            <!-- todo text -->
                                            <span class="text" id="val_<?=$row['id'];?>">
                                        <a href="#" data-pk="<?=$row['id'];?>" data-url="inc/main_portal/actions.php" id="edit_item" data-type="text" class="">
                                                <?=$row['name'];?>
                                            </a> 
                                            </span>
                ...
                <label style="padding-right:20px;">
                    <input id="make_main_manual" name="" value="<?=$row['id'];?>" type="checkbox" <?=$c;?>> <small><?=lang('PORTAL_on_main');?></small>
                </label>
                <i id="open_link" value="<?=$row['uniq_id'];?>" class="fa fa-external-link"></i> 
                <i id="edit_manual_cat" value="<?=$row['uniq_id'];?>" class="fa fa-pencil-square-o"></i>
            <i id="del_item" value="<?=$row['id']?>" class="fa fa-trash-o"></i>
...
//echo $tt['status'];
   	?>
              <div class="btn-group-vertical btn-block btn-xs">
                          <button type="button" id="make_post_status" value="0" class="btn <?=$status['0'];?> btn-default btn-xs"><?=lang('PORTAL_status_1');?></button>
                          <button type="button" id="make_post_status" value="1" class="btn <?=$status['1'];?> btn-warning btn-xs"><?=lang('PORTAL_status_2');?></button>
                          <button type="button" id="make_post_status" value="2" class="btn <?=$status['2'];?> btn-success btn-xs"><?=lang('PORTAL_status_3');?></button>
                          <button type="button" id="make_post_status" value="3" class="btn <?=$status['3'];?> btn-danger btn-xs"><?=lang('PORTAL_status_4');?></button>
                          <button type="button" id="make_post_status" value="4" class="btn <?=$status['4'];?> btn-primary btn-xs"><?=lang('PORTAL_status_5');?></button>
                        </div>
   	<?php
   }
...
//echo $tt['status'];
   	?>
              ...
              <div class="btn-group-vertical btn-block btn-xs">
                          <button type="button" id="make_post_status" value="0" class="btn <?=$status['0'];?> btn-default btn-xs"><?=lang('PORTAL_status_1');?></button>
                          <button type="button" id="make_post_status" value="1" class="btn <?=$status['1'];?> btn-warning btn-xs"><?=lang('PORTAL_status_2');?></button>
                          <button type="button" id="make_post_status" value="2" class="btn <?=$status['2'];?> btn-success btn-xs"><?=lang('PORTAL_status_6');?></button>
                          <button type="button" id="make_post_status" value="3" class="btn <?=$status['3'];?> btn-danger btn-xs"><?=lang('PORTAL_status_7');?></button>
                          <button type="button" id="make_post_status" value="4" class="btn <?=$status['4'];?> btn-primary btn-xs"><?=lang('PORTAL_status_5');?></button>
                        </div>
   	<?php
   }
...
if (check_validate() == true) {
?><div class="btn-group ">
                        <a href='<?php echo $CONF['hostname']; ?>sys/post.download.php?<?php echo $r['file_hash']; ?>' class="btn btn-sm btn-default"><?=lang('PORTAL_download');?></a>
                        <?php if (validate_user($_SESSION['helpdesk_user_id'], $_SESSION['code'])) { ?> 
                        <a id="delete_file" value="<?php echo $r['file_hash']; ?>" class="btn btn-sm btn-default"><?=lang('PORTAL_delete');?></a>
                        <?php }?> 
                        </div>
<?php } ?>
...
$stmt = $dbConnection->prepare('SELECT status,type FROM portal_posts where uniq_id=:id and parent_id=0');
    ...
    $status="";
    if ($post['type'] == 1 ) {
    	if ($post['status'] == "0") {       $status='<span class="label label-default">'.lang('PORTAL_status_1').'</span>';}
        else if ($post['status'] == "1") {  $status='<span class="label label-warning">'.lang('PORTAL_status_2').'</span>';}
        else if ($post['status'] == "2") {  $status='<span class="label label-success">'.lang('PORTAL_status_3').'</span>';}
        else if ($post['status'] == "3") {  $status='<span class="label label-danger">'.lang('PORTAL_status_4').'</span>';}
        else if ($post['status'] == "4") {  $status='<span class="label label-primary">'.lang('PORTAL_status_5').'</span>';}
        ...
        else if ($post['status'] == "2") {  $status='<span class="label label-success">'.lang('PORTAL_status_6').'</span>';}
        else if ($post['status'] == "3") {  $status='<span class="label label-danger">'.lang('PORTAL_status_7').'</span>';}
        else if ($post['status'] == "4") {  $status='<span class="label label-primary">'.lang('PORTAL_status_5').'</span>';}
        $status=$status.$c;
    }
