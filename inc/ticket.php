...
if (validate_user($_SESSION['helpdesk_user_id'], $_SESSION['code'])) {
    ...
    if (!empty($res1)) {
        foreach($res1 as $row) {
            ...
            if ($arch == 0) {
                if ($status_ok == 1) {$st=  "<span class=\"label label-success\"><i class=\"fa fa-check-circle\"></i> ".lang('TICKET_status_ok')." ".nameshort(name_of_user_ret_nolink($ok_by))."</span>";}
                if ($status_ok == 0) {
                    if ($lock_by <> 0) {$st=  "<span class=\"label label-warning\"><i class=\"fa fa-gavel\"></i> ".lang('TICKET_status_lock')." ".name_of_user_ret_nolink($lock_by)."</span>";}
            ...
            <section class="content-header">
                    <h1>
                        <i class="fa fa-ticket"></i> <?=lang('TICKET_name');?> <strong>#<?=$row['id']?></strong>
                        <small>
                        	<?=make_html($row['subj'], 'no')?>
                        </small>
                    </h1>
                    <ol class="breadcrumb">
						<li><a href="<?=$CONF['hostname']?>index.php"><span class="icon-svg"></span> <?=$CONF['name_of_firm']?></a></li>
                        <li class="active"><?=lang('TICKET_name');?> #<?=$row['id']?></li>
                    </ol>
                </section>
...
<div class="col-md-8">
	...
	<div class="col-md-12">
                    <div class="box">
                                ...
                                <time id="c" datetime="<?=$row['date_create']; ?>"></time></small>
                ...
                <tr>
                    ...
                    <td><small><?=name_of_user_ret($row['user_init_id'])?> </small></td>
			...
			<div class="text-muted well well-sm no-shadow" style="margin-top: 10px;">
                            ...
                            <div class="row"><div class="col-md-12">
	                            <a href="print_ticket?<?=$hn;?>" class="btn btn-default btn-xs pull-right"><i class="fa fa-print"></i> Print</a>
                            <?php if (($inituserid_flag == 1) && ($arch == 0)) { ?><button type="button" class="btn btn-default btn-xs pull-right" data-toggle="modal" data-target="#myModal"><?=lang('CONF_act_edit');?></button> <?php } ?>
                            </div>
                            </div>                            <?php 
...
<div class="box-body">
                ...
                <div class="form-group" id="t_for_to" data-toggle="popover" data-html="true" data-trigger="manual" data-placement="right" data-content="<small><?=lang('NEW_to_unit_desc');?></small>">
                    <label for="t_to" class="col-sm-3 control-label"><small><?=lang('TICKET_t_refer_to');?>: </small></label>
...
//echo($row['label']);
    ...
    <select data-placeholder="<?=lang('NEW_to_user');?>"  id="t_users_do" name="unit_id" class="form-control input-sm" multiple>
    	<option></option>
...
//echo($row['label']);
                                ...
                                <option value="<?=$row['value']?>"><?=nameshort($row['label'])?></option>
...
<div class="nav-tabs-custom">
                                <ul class="nav nav-tabs">
                                    <li class="active"><a href="#tab_1" data-toggle="tab"><i class="fa fa-comments-o"></i> <?=lang('TICKET_t_comment');?></a></li>
                                    <li class=""><a href="#tab_2" data-toggle="tab"><?=lang('TICKET_t_history');?></a></li>
                                ...
                                <div class="box-footer">
                                    <div class="input-group" id="for_msg">
                                        <input name="msg" id="msg" class="form-control" data-toggle="popover" data-html="true" data-trigger="manual" data-placement="top" data-content="&lt;small&gt;<?=lang('TICKET_t_det_ticket');?>&lt;/small&gt;" placeholder="<?=lang('TICKET_t_comm_ph');?>"/>
                                        <div class="input-group-btn">
                                            <button value="<?=$hn?>" id="do_comment" class="btn btn-success"><i class="fa fa-comment"></i></button>
...
<div class="callout callout-danger">
                                        <h4><?=lang('MAIN_attention');?></h4>
                                        <p><?=lang('TICKET_t_in_arch');?></p>
                                    </div>
...
<div class="callout callout-warning">
                                        <h4><?=lang('MAIN_attention');?></h4>
                                        <p><i class="fa fa-check-circle"></i> <?=lang('TICKET_t_ok');?> <strong> <?=name_of_user_ret($ok_by)?></strong> <?=$ok_date;?>.<br> <?=lang('TICKET_t_ok_1');?></p>
                                    </div>
                ...
                if ($status_ok == 0) {
                    if ($lock_by <> 0) {
                        if ($status_lock == "you") {
                            ...
                            <div class="callout callout-warning">
                                        <h4><?=lang('MAIN_attention');?></h4>
                                        <p><i class="fa fa-check-circle"></i> <?=lang('TICKET_t_lock');?> <strong> <?=name_of_user_ret($lock_by)?></strong> .<br> <?=lang('TICKET_t_lock_1');?></p>
                                    </div>
                        ...
                        if ($status_lock == "me") {
                            ...
                            <div class="callout callout-warning">
                                        <h4><?=lang('MAIN_attention');?></h4>
                                        <p><i class="fa fa-check-circle"></i> <?=lang('TICKET_t_lock_i');?></p>
                                    </div>
            ...
            <input type="hidden" id="ticket_id" value="<?=$tid;?>">
...
<div class="control-group" id="for_s">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control input-sm" name="subj" id="subj" placeholder="<?=lang('NEW_subj');?>" value="<?=$subj;?>">
    </div>
  </div></div></div>
<?php } 
...
<div class="control-group" id="for_subj" >
    <div class="controls">
        ...
        foreach($res11 as $rows) {
					$sel_flag="";
					if ($rows['name'] == $subj) {$sel_flag="selected";}
                        ?>
