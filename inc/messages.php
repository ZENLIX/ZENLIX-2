...
<section class="content-header">
                    <h1>
                        <i class="fa fa-comments"></i> <?=lang('MESSAGES_us');?>
                        <small><?=lang('MESSAGES_us_ext');?></small>
                    </h1>
                    <ol class="breadcrumb">
                       <li><a href="<?=$CONF['hostname']?>index.php"><span class="icon-svg"></span> <?=$CONF['name_of_firm']?></a></li>
                        <li class="active"><?=lang('MESSAGES_us');?></li>
                    </ol>
                </section>
...
<div class="col-md-3">
	<div class="row">
		...
		<div class="col-md-12">
			<a id="select_main_chat" class="btn btn-block btn-default" ><?=lang('MESSAGES_main');?></a>
			<br>
		</div>
		...
		<div class="box box-info">
                                ...
                                <div class="box-body">
                                <?php if ($_GET['to']) { $t=$_GET['to']; $ufio=get_user_val_by_hash($t,'fio'); }
                                else {$ufio="";} ?>
                                
	                                <input class="form-control input-sm" id="find_user" type="text" placeholder="<?=lang('MESSAGES_fio');?>" value="<?=$ufio;?>">
                                    <?php
...
$tt=$stmt->fetch(PDO::FETCH_ASSOC);
	 ...
	 if ($_GET['to']) { $t=$_GET['to']; $tuid=get_user_val_by_hash($t,'id'); 
		 
		 if ($tuid == $uniq_id) {
		 ?>
                                                    <li class="user_li active" user-id="<?=$uniq_id;?>">
                                                    <a href="#">
	                                                    <img style="width: 25px;
height: 25px;" src="<?=get_user_img_by_id($uniq_id);?>" class="img-circle" alt="User Image">
		...
		<div class="row">
			<div class="col-md-12"><div class="box box-solid box-default" style="margin-bottom: 2px; height:100%;">
                                <div class="box-header">
                                    ...
                                    <?php 
                                   if ($_GET['to']) { $t=$_GET['to']; 
                                   ?>
                                   <?=lang('MESSAGES_with');?> <?=get_user_val_by_hash($t,'fio');?>
									   
<?php }
else { ?><?=lang('MESSAGES_main');?><?php } ?>
			...
			<div class="col-md-12"><div class="box box-solid">
                                ...
                                <div class="box-body">
                                   ...
                                   <div class="input-group" id="for_msg">
                                        <input name="msg" id="msg" class="form-control" data-toggle="popover" data-html="true" data-trigger="manual" data-placement="top" data-content="<?=lang('MESSAGES_sel_text');?>" placeholder="<?=lang('MESSAGES_sel_text');?>">
                                   ...
                                   if ($_GET['to']) { $t=$_GET['to']; 
                                   ?>
                                   <input type="hidden" id="target_user" value="<?=get_user_val_by_hash($t,'id');?>">
                                   <input type="hidden" id="total_msgs_main" value="<?=get_total_msgs_main();?>">
									   
<?php }
else { ?><input type="hidden" id="target_user" value="main">
<input type="hidden" id="total_msgs_main" value="<?=get_total_msgs_main();?>"><?php } ?>
