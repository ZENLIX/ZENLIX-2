...
if ( isset($_POST['mode']) ) {
    ...
    if ($mode == "get_lang_param") {
	    ...
	    if (validate_exist_login($login) == false) {$errors=true;
	    $el=lang('ticket_login_error')."<br>";
		    
	    }
	    if (!validate_email($mail)) {$errors=true;
		$el.=lang('PROFILE_msg_error')."<br>";
	    }
        if (validate_exist_mail_not_auth($mail) == false) {$errors=true;
        ...
        if ($mode == "get_list_notes") {
            ...
            if (empty($res)) {
                ...
                foreach($res as $row) {
                    ...
                    <li class="tr_<?=$row['id'];?>">
<a style=" cursor: pointer; " id="to_notes" value="<?=$row['hashname'];?>"><?=$t_msg;?>

<span class="badge pull-right bg-red" id="del_notes" value="<?=$row['hashname'];?>">
<i class="glyphicon glyphicon-trash"></i></span>
        ...
        if ($_POST['exclude']) {
	        ...
	        $t=$_POST['exclude'];
	    if (validate_exist_login_ex($l, $t) == true) {$r['check_login_status']=true;}
        else if (validate_exist_login_ex($l, $t) == false) {$r['check_login_status']=false;}
        }
	...
	$get_total_tickets_no_ok=$count[0];
                        ...
                        <div class="box box-info">
	                        <div class="box-header">
                                    <h4 class="box-title"><?=lang('EXT_stat_title');?> <time id="c" datetime="<?=$start?>"></time> - <time id="c" datetime="<?=$end?>"></time></h4>
                                </div>
	                        <div class="panel-body" style="max-height: 400px; scroll-behavior: initial; overflow-y: scroll;">
                                        ...
                                        <table class="table table-hover">
                                            ...
                                            <tr>
                                                ...
                                                <th><center><small><?=lang('TICKET_name');?>	</small></center></th>
...
$ru=name_of_user_ret($row['init_user_id']);
                                                ...
                                                <tr>
                                                    ...
                                                    <td style=" width: 70px; vertical-align: inherit;"><center><small>
	                                                   <a href="ticket?<?=get_ticket_hash_by_id($row['ticket_id'])?>"> #<?=$row['ticket_id']?></a>
	                                                    </small></center></td>
                        ...
                        <div class="box-body"></div></div>
	                        ...
	                        <div class="box box-info">
		                        <div class="box-header">
                                    <h4 class="box-title"><?=lang('EXT_stats_main');?></h4>
                                </div>
                                
		                        <div class="row">
			                <div class="col-xs-4 text-center" style="border-right: 1px solid #f4f4f4">
                                            <input type="text" class="knob" data-readonly="true" value="<?=$get_total_tickets_create;?>" data-width="100" data-height="100" data-max="50" data-max="<?=(get_total_tickets_count());?>" data-fgColor="#39CCCC"/>
                                            <div class="knob-label"><?=lang('EXT_t_created');?></div>
                                        </div><!-- ./col -->
		                    <div class="col-xs-4 text-center" style="border-right: 1px solid #f4f4f4">
                                            <input type="text" class="knob" data-readonly="true" value="<?=$get_total_tickets_refer;?>" data-width="100" data-height="100" data-max="50" data-max="<?=(get_total_tickets_count());?>" data-fgColor="#932AB6"/>
                                            <div class="knob-label"><?=lang('EXT_stats_refer');?></div>
                                        </div><!-- ./col -->
			                ...
			                <div class="col-xs-4 text-center" style="border-right: 1px solid #f4f4f4">
                                            <input type="text" class="knob" data-readonly="true" value="<?=$get_total_tickets_lock;?>" data-width="100" data-height="100" data-max="50" data-max="<?=(get_total_tickets_count());?>" data-fgColor="#F39C12"/>
                                            <div class="knob-label"><?=lang('EXT_stats_lock');?></div>
                                        </div><!-- ./col -->
		                    <div class="col-xs-4 text-center" style="border-right: 1px solid #f4f4f4">
                                            <input type="text" class="knob" data-readonly="true" value="<?=$get_total_tickets_unlock;?>" data-width="100" data-height="100" data-max="50" data-max="<?=(get_total_tickets_count());?>" data-fgColor="#001F3F"/>
                                            <div class="knob-label"><?=lang('EXT_stats_unlock');?></div>
                                        </div><!-- ./col -->
                            <div class="col-xs-4 text-center">
                                            <input type="text" class="knob" data-readonly="true" value="<?=$get_total_tickets_no_ok;?>" data-width="100" data-height="100" data-max="50" data-max="<?=(get_total_tickets_count());?>" data-fgColor="#F56954"/>
                                            <div class="knob-label"><?=lang('EXT_stats_no_ok');?></div>
                                        </div><!-- ./col -->
		                        </div>
	                        </div>
<?php
...
//print_r($data);
//echo $getver;
if ($myversion >= $getver) {echo "<br><center>"."You have latest version."."</center>";}
else if ($myversion < $getver) {echo "<br><center>".$data['msg']."</center><br>";
...
$r['p']=$row['id'];
 ...
 if ($tc == "1") {
                ...
                get_client_info($client_id);
	                ...
	                ?>
	                <?=get_client_info_ticket($client_id)?>
	                <?php
                }
...
<div class="box box-info">
                                <div class="box-header">
                                    ...
                                    <h3 class="box-title"> <?=lang('WORKER_TITLE');?></h3>
                                </div><!-- /.box-header -->
                                <div class="box-body" >
...
$l=$_POST['value'];
		...
		if (validate_exist_login($l) == true) {echo "";}
...
if ($r != 0) { echo $r; } else if ($r == 0) { echo "";}
...
if ($mode == "view_unread_msgs") {
	$stmt = $dbConnection->prepare('SELECT user_from, msg, date_op from messages where user_to=:uto and is_read=0');
                                    ...
                                    ?>
                                        ...
                                        <li><!-- start message -->
                                            <a href="messages?to=<?=get_user_val_by_id($rews['user_from'],'uniq_id');?>">
                                                <div class="pull-left">
                                                    <img src="<?=get_user_img_by_id($uniq_id);?>" class="img-circle" alt="User Image"/>
                                                </div>
                                                <h4>
                                                    <?=nameshort(name_of_user_ret_nolink($rews['user_from']));?>
                                                    
                                                    <small><i class="fa fa-clock-o"></i> <time id="b" datetime="<?=$rews['date_op'];?>"></time> </time></small>
                                                </h4>
                                                <p><?=make_html($rews['msg'], 'no');?></p>
                                            </a>
                                        </li><!-- end message -->
                                        <?php
...
if ($mode == "show_online_users") {
    ...
    ?>
<li><!-- start message -->
                                            <a href="view_user?<?=$rews['uniq_id'];?>">
                                                <div class="pull-left">
                                                    <img src="<?=get_user_img_by_id($rews['id']);?>" class="img-circle" alt="User Image"/>
                                                </div>
                                                <h4>
                                                    <?=nameshort(name_of_user_ret_nolink($rews['id']));?>
                                                    
                                                    
                                                </h4>
                                                <p><?=get_user_val_by_id($rews['id'],'posada');?></p>
                                            </a>
                                        </li><!-- end message -->
                                       <?php  } 
...
if (get_user_status_text($row['value']) == "online") {$s="online";}
        ...
        if ($mode == "edit_helper") {
			...
			if ($isclient_status == "1") { $isclient_status="checked";} else {$isclient_status="";}
...
//
                ...
                <div class="">
                    <div class="">
                        ...
                        <div class="form-group">
  <label for="is_client" class="col-sm-2 control-label"><small><?=lang('EXT_for_clients');?></small></label>
  <div class="col-sm-10">
	  ...
	  <div class="checkbox">
    <label>
      <input type="checkbox" id="is_client" <?=$isclient_status;?>> <?=lang('CONF_true');?>
      <p class="help-block"><small><?=lang('EXT_for_clients_ext');?></small></p>
    </label>
  </div>
	  </div>
  </div>
  ...
  <div class="col-sm-10">
	  ...
	  <div class="checkbox">
    <label>
      <input type="checkbox" id="is_client"> <?=lang('CONF_true');?>
      <p class="help-block"><small><?=lang('EXT_for_clients_ext');?></small></p>
    </label>
  </div>
	  </div>
	 ...
	 $stmt = $dbConnection->prepare("SELECT 
            ...
            foreach($result as $row) {
                ...
                if ($ac == "ok") {
                    ...
                    <div class="box box-solid">
                                <div class="box-header">
                                    <h5 class="box-title"><small><i class="fa fa-file-text-o"></i></small> <a style="font-size: 18px;" class="text-light-blue" href="helper?h=<?=$row['hashname'];?>"><?=$row['title'];?></a></h5>
                                    <div class="box-tools pull-right">

                                    </div>
                                </div>
                                <div class="box-body">
                                    <small><?=cutstr_help_ret(strip_tags($row['message']));?>
                            </small>                                </div><!-- /.box-body -->
                            </div>
                <?php
                }
                ...
                if ($ac == "ok") {
                    ...
                    <div class="box box-solid">
                                <div class="box-header">
                                    <h5 class="box-title"><small><i class="fa fa-file-text-o"></i></small> <a style="font-size: 18px;" class="text-light-blue" href="helper?h=<?=$row['hashname'];?>"><?=$row['title'];?></a></h5>
                                    <div class="box-tools pull-right">
<small>(<?=lang('DASHBOARD_author');?>: <?=nameshort(name_of_user_ret($row['user_init_id']));?>)<?php if ($priv_h== "yes") { echo " 
	        <div class=\"btn-group\">
            ...
            <?php
                ...
                ?>
                 <p class="text-muted well well-sm no-shadow" style="margin-top: 10px;">
                    ...
                    <center><?=lang('MSG_no_records');?></center></p>
...
<small>(<?=lang('DASHBOARD_author');?>: <?=nameshort(name_of_user_ret($row['user_init_id']));?>)<?php if ($priv_h== "yes") { echo " 
        ...
        if ($mode == "do_save_help") {
			...
			if ($is_client == "true") {$is_client=1;} else {$is_client=0;}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    ...
                    href="ticket?<?php echo $row['hash_name']; ?>"><?php cutstr(make_html($row['subj'], 'no')); ?></a></td>
                        ...
                        <a href="view_user?<?=get_user_hash_by_id($row['client_id']);?>">
                        <?= get_user_val_by_id($row['client_id'], 'fio'); ?>
                        </a>
                        </small></td>
                        ...
                        <td style=" vertical-align: middle; "><small class="<?=$muclass;?>">
                        <a href="view_user?<?=get_user_hash_by_id($row['user_init_id']);?>">
...
//update
 ...
 switch($type_op) {
	case 'ticket_create': 	$at=get_last_msg_ticket($ticket_id, 'create');		break;
	case 'ticket_refer':	$at=get_last_msg_ticket($ticket_id, 'refer');		break;
	
	case 'ticket_ok':		$at=get_last_msg_ticket($ticket_id, 'ok');			break;
	case 'ticket_no_ok':	$at=get_last_msg_ticket($ticket_id, 'no_ok');		break;
	case 'ticket_lock':		$at=get_last_msg_ticket($ticket_id, 'lock');		break;
	case 'ticket_unlock':	$at=get_last_msg_ticket($ticket_id, 'unlock');		break;
	case 'ticket_comment':	$at=get_last_msg_ticket($ticket_id, 'comment');		break;
	}
	...
	$results[] = array(
                            'url' => $CONF['hostname'],
                            'up' => lang('JS_up'), 														//обновлено
                            'ticket' => lang('JS_ticket'),												//Заявка
                            'name' => $ticket_id,
                            'at' => $at,																//слова
                            'hash' => get_ticket_hash_by_id($ticket_id),
                            'time' => "<time id=\"b\" datetime=\"".date("Y-m-d H:i:s")."\"></time>" 	//время
								);
...
if ($mode == "conf_edit_pb") {
        ...
        if ($mode == "del_profile_img") {
            ...
            if ($ec == 0) {
                ...
                $stmt->execute(array(':id' => $id, ':m' => $m, ':langu' => $langu, ':s' => $skype, ':t' => $tel,
                ':adr'=>$adr, ':fio'=>$fio));
        ...
        if ($mode == "edit_profile_main") {
            ...
            if ($ec == 0) {
                ...
                adr=:adr,posada=:posada,unit_desc=:unitss where id=:id');
                $stmt->execute(array(':id' => $id, ':m' => $m, ':langu' => $langu, ':s' => $skype, ':t' => $tel,
                ':adr'=>$adr,':posada'=>$posada,':unitss'=>$unitss, ':fio'=>$fio));
...
if ($priv == "4") { $is_client="1"; $privs="1";}
else if ($priv != "4") {$is_client="0"; $privs=$priv;}
...
$tt=$stmt->fetch(PDO::FETCH_ASSOC);
        if ($tt['cou'] != 0 ) {$tt= "<small class=\"badge pull-right\">".$tt['cou']."</small>"; }
        else {$tt="";}
...
$tt=$stmt->fetch(PDO::FETCH_ASSOC);
        if ($tt['cou'] != 0 ) {$tt= "<small id=\"ul_label_".$uniq_id."\"><small class=\"badge pull-right\">".$tt['cou']."</small></small>"; }
        else {$tt="<small id=\"ul_label_".$uniq_id."\"></small>";}

			                                               
			                                               
	                                                ?>
                                                    <li class="user_li" user-id="<?=$uniq_id;?>">
                                                    <a href="#">
	                                                    <img style="width: 25px;height: 25px;" src="<?=get_user_img_by_id($uniq_id);?>" class="img-circle" alt="User Image">
                                                     <?=nameshort(name_of_user_ret_nolink($uniq_id));?>
...
<?php
        ...
        if ($target == "main") {$a="0"; $b="main";}
        else if ($target != "main") {$a=$target; $b="0";
